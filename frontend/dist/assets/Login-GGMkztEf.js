import{a as e}from"./rolldown-runtime-CNC7AqOf.js";import{c as t,s as n}from"./motion-vendor-CYHVKki_.js";import{a as r,d as i,o as a,s as o}from"./index-DUk1bxXA.js";var s=e(t(),1),c=()=>{let[e,t]=(0,s.useState)(``),[n,c]=(0,s.useState)(!1),l=i(),{setUser:u}=r(),d=e=>({id:e.id||e.user?.id,name:e.name||e.user?.name,email:e.email||e.user?.email,avatar:e.avatar||e.user?.avatar,goal:e.goal||e.user?.goal});return{error:e,loading:n,handleSubmit:async(e,{email:n,password:r})=>{e.preventDefault(),t(``),c(!0);try{let e=await fetch(`${a}/api/auth/login`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({email:n,password:r})}),t=await e.json();if(!e.ok)throw Error(t.message||`Login failed`);let i=d(t);if(!i.id)throw Error(`Login response did not include a user ID.`);u(i),l(`/dashboard`)}catch(e){t(e.message)}finally{c(!1)}},loginWithGoogle:o({onSuccess:async e=>{t(``),c(!0);try{let t=await fetch(`${a}/api/auth/google-login`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({code:e.code})}),n=t.headers.get(`content-type`);if(!n||!n.includes(`application/json`))throw Error(`Server response was not JSON. Check backend console.`);let r=await t.json();if(!t.ok)throw Error(r.message||`Google synchronization failed.`);let i=d(r);if(!i.id)throw Error(`Google login response did not include a user ID.`);u(i),l(`/dashboard`)}catch(e){console.error(`Google Login Error:`,e),t(e.message)}finally{c(!1)}},onError:()=>t(`Google Authentication Interrupted.`),flow:`auth-code`})}},l=n(),u=`#8FBF63`,d=`#9DCB72`,f=`rgba(143, 191, 99, 0.22)`,p=`rgba(143, 191, 99, 0.05)`,m=()=>{let[e,t]=(0,s.useState)(``),[n,r]=(0,s.useState)(``),[a,o]=(0,s.useState)(``),m=i(),{error:h,loading:g,handleSubmit:_,loginWithGoogle:v}=c();return(0,l.jsxs)(`div`,{"data-theme":`dark`,className:`relative min-h-screen min-h-[100dvh] w-full bg-[#0e0e0e] font-['DM_Sans'] text-[#e5e2e1] overflow-x-hidden`,children:[(0,l.jsxs)(`button`,{type:`button`,onClick:()=>m(`/`),className:`fixed top-5 left-5 sm:top-8 sm:left-8 z-50 flex items-center gap-2 px-4 py-2.5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-md text-[11px] font-semibold tracking-widest uppercase text-white/60 hover:text-white hover:bg-white/10 hover:border-white/20 transition-all`,children:[(0,l.jsx)(`span`,{className:`text-base leading-none`,children:`←`}),`Back`]}),(0,l.jsx)(`style`,{children:`
        .vitalis-vignette {
