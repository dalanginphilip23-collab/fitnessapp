...
`.trim();
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/coach`, {
          ...
          body: JSON.stringify({
            system:
              "You are a terse, encouraging personal trainer. Reply with ONE coaching cue of 12 words or fewer. No quotes, no trailing punctuation.",
            prompt: userPrompt,
          }),
        });
        ...
        const fb = isBlocked
          ? blockedFallbacks[
              Math.floor(Math.random() * blockedFallbacks.length)
            ]
  ...
  const maybeAnalyze = useCallback(
    (landmarks) => {
      const now = Date.now();
