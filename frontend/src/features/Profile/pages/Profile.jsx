    ...
    <div className="min-h-screen bg-(--bg-primary) text-(--text-primary) font-['Poppins'] flex overflow-x-hidden">
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-400 ease-in-out ${expanded ? 'md:ml-60' : 'md:ml-18'}`}>
        ...
        <main className="w-full max-w-[1400px] mx-auto px-3 sm:px-4 md:px-6 lg:px-8 pt-6 sm:pt-8 mt-14 sm:mt-15 pb-28 md:pb-16">
          ...
          {/* ── Two-column shell: persistent identity sidebar (left) + tabbed content (right) ── */}
          <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-3 items-start">

            {/* ══════════════ LEFT: identity + always-visible summary ══════════════ */}
            <div className="flex flex-col gap-3 lg:sticky lg:top-20">

              {/* Identity card */}
              <div className="bg-(--bg-card) border border-(--border-light) rounded-2xl p-5 sm:p-6 flex flex-col items-center text-center">
                {/* Avatar */}
                ...
                <h1 className="mt-4 text-xl font-['Manrope'] font-black text-(--text-primary) tracking-tight truncate max-w-full">
                  {formData.fullName || 'Your Name'}
                </h1>

                <span className="mt-2 flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wider px-2.5 py-1 rounded-full bg-(--bg-hover) text-(--text-muted) border border-(--border-light) shrink-0">
                  <span className="material-symbols-outlined text-[11px]">badge</span>
                  ID {recordId}
                </span>

                <p className="flex items-center gap-1.5 mt-3 text-[11px] text-(--text-muted) font-medium truncate max-w-full">
                ...
                <button
                  ...
                  className={`w-full mt-4 flex items-center justify-center gap-1.5 text-[10px] font-bold uppercase tracking-wider border rounded-xl px-3.5 py-2 transition-all ${
                    isEditing
                ...
                {isDirty && (
                  <div className="w-full flex flex-col gap-2 mt-4 pt-4 border-t border-(--border-light)">
                    <span className="flex items-center justify-center gap-1.5 text-[9px] font-bold text-amber-400 uppercase tracking-wider">
                      <span className="w-1 h-1 rounded-full bg-amber-400 animate-pulse" />
                      Unsaved changes
                    </span>
                    <div className="flex items-center gap-2">
                      <button onClick={handleDiscard} className="flex-1 px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider text-(--text-muted) hover:text-(--text-secondary) rounded-lg hover:bg-(--bg-hover) transition-all">
                        Discard
                      </button>
                      <button onClick={handleSave} disabled={isSaving} className="flex-1 flex items-center justify-center gap-1.5 bg-[#62aa1a] text-[#161f00] px-3.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider active:scale-95 transition-all disabled:opacity-60 hover:brightness-105">
              ...
              {/* Avatar picker panel — kept next to the avatar it edits */}
              {pickerOpen && (
                <div className="bg-(--bg-card) border border-(--border-light) rounded-2xl p-5" style={{ animation: 'fadeIn 0.15s ease' }}>
                  <div className="flex items-center justify-between mb-4">
              ...
              {/* Health overview — always visible, independent of active tab */}
              ...
              {/* Account — always visible, independent of active tab */}
              <div className="bg-(--bg-card) border border-(--border-light) rounded-2xl p-5">
            ...
            <div className="flex flex-col gap-3 min-w-0">
              ...
              {/* Tabs */}
              <div className="flex bg-(--bg-card) border border-(--border-light) rounded-2xl px-3 sm:px-5 pt-1.5 gap-1">
                {[
